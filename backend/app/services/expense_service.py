...
class ExpenseService:
    ...
    async def create_expense(self, session: AsyncSession, data: ExpenseCreate) -> ExpenseResponse:
        ...
        try:
            ...
            # Duplicate UUID → idempotent retry. Return existing record.
    ...
    async def list_expenses(
        ...
        expenses = await self.repo.list_all(session, category=category, sort=sort)
        total_paisa = await self.repo.get_total_paisa(session, category=category)
